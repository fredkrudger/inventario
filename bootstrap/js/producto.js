...
<<<<<<< HEAD
                    ...
                    cantidad_minima:{
                        required  : '<b style="color:red">Debes Ingresar una cantidad minima.</b>',
                        minlength : '<b style="color:red">Ingresa al menos 1 digito.</b>',
                        maxlength : '<b style="color:red">La cantidad minima excede de digitos.</b>'
                    },
...
<<<<<<< HEAD
    ...
    $("#results-div").load("listado_grupos.php"); 
    $("#results-div").on( "click", ".paginacion a", function (e){
    e.preventDefault();
    $(".listado-div").show(); 
    var page = $(this).attr("data-page"); 
    $("#results-div").load("listado_grupos.php",{"page":page}, function(){
      $(".listado-div").hide();
     });
...
<<<<<<< HEAD
   ...
   $("#grupo_nuevo").validate({          
      ...
      messages:{
                grupo_new:{
                        required  : '<b style="color:red">Debes Ingresar un grupo.</b>', 
                        minlength : '<b style="color:red">Ingresa al menos 4 caracteres.</b>',
                        maxlength : '<b style="color:red">El grupo excede de caracteres.</b>'
                 },            
                },
          ...
          submitHandler: function(form){          
            var url = "guardar_producto.php";
                  $.ajax({
                   type: "POST",
                   url: url,
                   data: $("#grupo_nuevo").serialize(),
                   success: function(data)
                   {  
                     $("#resp_grupo").html(data);
                   }
               }); 
            return false;
          }
...
<<<<<<< HEAD
                    html+='Bs. ' + json.data[datos].precio+'       ';
                            ...
                            html+='<a style="margin-left:5px" title="Eliminar Producto" class="btn btn-sm btn-danger" href="javascript:eliminar('+json.data[datos].codigo_producto+')"> Eliminar</a>';
                             } 
=======
                            html+='Bs. ' + json.data[datos].precio+'       ';
...
function editarGrupo(id){
   var url = 'editar_grupo.php';
      ...
      data: id,
      success: function(data){
       window.location="index.php?editar_grupo="+id; 
      }
   })     
}
...
function eliminarGrupo(id){ 
  var url = 'eliminar_grupo.php';
  var pregunta = confirm('¿Estas seguro que deseas eliminar el grupo?');
   if(pregunta==true){
    $.ajax({
           type: "POST",
           url: url,
           ...
           success: function()
           {
           window.location="index.php?grupos"
           }
        });
    }  
}
